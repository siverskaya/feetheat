...
All models for the database and its three main tables (listings, orders, users) are found at feetheat/app/models.
Includes associations/relationships between the tables (one user can have many listings, each listing is tied to only
one user, etc.) and validation of table data as well as image content.
